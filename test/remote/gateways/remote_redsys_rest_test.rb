...
class RemoteRedsysRestTest < Test::Unit::TestCase
  def setup
    ...
    @gateway_3ds = RedsysRestGateway.new(fixtures(:redsys_3ds))
    @amount = 100
  ...
  def test_successful_purchase_threeds1_as_mpi
    xid = '97267598-FAE6-48F2-8083-C23433990FBC'
    cavv = '123'
    eci = '01'
    version = '0.0.0'
  ...
  def test_successful_purchase_3ds
    ...
    response = @gateway_3ds.purchase(@amount, @threeds2_credit_card, options)
    assert_success response
  ...
  def test_successful_3ds2_purchase_with_mit_exemption
    options = @options.merge(execute_threed: true)
    response = @gateway_3ds.purchase(@amount, @threeds2_credit_card, options.merge(sca_exemption: 'MIT', sca_exemption_direct_payment_enabled: true))
    assert_success response
    assert response.params['ds_emv3ds']
  ...
  def test_failed_3ds2_purchase_with_mit_exemption_but_missing_direct_payment_enabled
    options = @options.merge(execute_threed: true)
    response = @gateway_3ds.purchase(@amount, @threeds2_credit_card, options.merge(sca_exemption: 'MIT'))
    assert_success response
    assert response.params['ds_emv3ds']
    assert response.params['ds_card_psd2']
    assert_equal '2.1.0', JSON.parse(response.params['ds_emv3ds'])['protocolVersion']
  ...
  def test_failed_void
    ...
    another_void = @gateway.void(authorize.authorization, @options)
